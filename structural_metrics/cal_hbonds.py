...
def _get_bond_triplets_(topology, input_reswatndx, sidechain_only=False): 
    """
    input_reswatndx: Must be list of strings !!! edited by Jingjing 2018-02-27
...
def md_cal_hbonding(traj, input_reswatndx, distance_cutoff=0.35,angle_cutoff=0,
    ...
    import numpy as np
    """
    Note: turn sidchain_only to True may have some problems; be careful !!!
    ...
    ## + acceptor_resnm + acceptor_resid + acceptor_atom + distance + angle
        for p_h, d_h, a_h in zip(pairs_h,dist_h,ang_h):
            outputfile.write('frame_'+str(frameID)+" "+str(p_h[0])+" "+str(p_h[2])+" "+\
                         ...
                         str(wat_pro_all_dict[str(p_h[0])][0])[3:]+" "+\
                         str(topology.atom(p_h[1])).split('-')[-1]+" "+\
                         str(wat_pro_all_dict[str(p_h[2])][0])[:3]+" "+\
                         str(wat_pro_all_dict[str(p_h[2])][0])[3:]+" "+\
                         str(topology.atom(p_h[2])).split('-')[-1]+" "+\
                         str(d_h)+" "+str(a_h)+"\n")
...
if __name__ == "__main__":
    ...
    wat_all_list=np.load('temp_wat_all_list.npy')  ### water resid list 
